...
describe('MenuItem Management Detail Component', () => {
      ...
      providers: [
        provideRouter(
          [
            {
              path: '**',
              loadComponent: () => import('./menu-item-detail.component').then(m => m.MenuItemDetailComponent),
              resolve: { menuItem: () => of({ id: 11248 }) },
            },
          ],
  ...
  describe('OnInit', () => {
    it('should load menuItem on init', async () => {
      const harness = await RouterTestingHarness.create();
      const instance = await harness.navigateByUrl('/', MenuItemDetailComponent);

      // THEN
      expect(instance.menuItem()).toEqual(expect.objectContaining({ id: 11248 }));
    });
  });
