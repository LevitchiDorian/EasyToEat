...
type RestOf<T extends IReservation | NewReservation> = Omit<
  T,
  'reservationDate' | 'reminderSentAt' | 'confirmedAt' | 'cancelledAt' | 'createdAt' | 'updatedAt'
> & {
...
export class ReservationService {
  ...
  protected convertDateFromClient<T extends IReservation | NewReservation | PartialUpdateReservation>(reservation: T): RestOf<T> {
    return {
      ...
      reminderSentAt: reservation.reminderSentAt?.toJSON() ?? null,
      confirmedAt: reservation.confirmedAt?.toJSON() ?? null,
      cancelledAt: reservation.cancelledAt?.toJSON() ?? null,
      createdAt: reservation.createdAt?.toJSON() ?? null,
      updatedAt: reservation.updatedAt?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restReservation: RestReservation): IReservation {
    return {
      ...
      reminderSentAt: restReservation.reminderSentAt ? dayjs(restReservation.reminderSentAt) : undefined,
      confirmedAt: restReservation.confirmedAt ? dayjs(restReservation.confirmedAt) : undefined,
      cancelledAt: restReservation.cancelledAt ? dayjs(restReservation.cancelledAt) : undefined,
      createdAt: restReservation.createdAt ? dayjs(restReservation.createdAt) : undefined,
      updatedAt: restReservation.updatedAt ? dayjs(restReservation.updatedAt) : undefined,
    };
  }
