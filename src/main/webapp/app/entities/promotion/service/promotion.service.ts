...
type RestOf<T extends IPromotion | NewPromotion> = Omit<T, 'startDate' | 'endDate'> & {
...
export class PromotionService {
  ...
  protected convertDateFromClient<T extends IPromotion | NewPromotion | PartialUpdatePromotion>(promotion: T): RestOf<T> {
    return {
      ...promotion,
      startDate: promotion.startDate?.format(DATE_FORMAT) ?? null,
      endDate: promotion.endDate?.format(DATE_FORMAT) ?? null,
    };
  }
  ...
  protected convertDateFromServer(restPromotion: RestPromotion): IPromotion {
    return {
      ...restPromotion,
      startDate: restPromotion.startDate ? dayjs(restPromotion.startDate) : undefined,
      endDate: restPromotion.endDate ? dayjs(restPromotion.endDate) : undefined,
    };
  }
