...
describe('Notification e2e test', () => {
  ...
  const notificationSample = {
    ...
    isRead: false,
    createdAt: '2026-02-22T18:05:39.004Z',
  };
  ...
  describe('new Notification page', () => {
    ...
    it('should create an instance of Notification', () => {
      cy.get(`[data-cy="type"]`).select('ORDER_STATUS_CHANGED');

      cy.get(`[data-cy="channel"]`).select('EMAIL');

      cy.get(`[data-cy="subject"]`).type('unless onto yin');
      cy.get(`[data-cy="subject"]`).should('have.value', 'unless onto yin');
      ...
      cy.get(`[data-cy="sentAt"]`).type('2026-02-23T12:09');
      cy.get(`[data-cy="sentAt"]`).blur();
      cy.get(`[data-cy="sentAt"]`).should('have.value', '2026-02-23T12:09');

      cy.get(`[data-cy="readAt"]`).type('2026-02-23T10:48');
      cy.get(`[data-cy="readAt"]`).blur();
      cy.get(`[data-cy="readAt"]`).should('have.value', '2026-02-23T10:48');

      cy.get(`[data-cy="createdAt"]`).type('2026-02-23T12:05');
      cy.get(`[data-cy="createdAt"]`).blur();
      cy.get(`[data-cy="createdAt"]`).should('have.value', '2026-02-23T12:05');
